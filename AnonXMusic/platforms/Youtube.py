...
# Updated: 3-Tier Hybrid Search / RCE Secured globally / Accurate Telemetry
...
# ============================
# 🛡 UNIVERSAL INPUT FIREWALL 🛡
# ============================
def is_safe_url(text: str) -> bool:
    if not text: return False
    text = str(text).strip()
    
    # 1. Check for command injection variables universally
    try:
        decoded_text = unquote(text).lower()
        LETHAL_PATTERNS = [";", "|", "`", "${", "$ifs", "$(", "\n", "\r", "/proc/", "curl ", "wget ", "tar ", "cp "]
        if any(p in decoded_text for p in LETHAL_PATTERNS):
            LOGGER.warning(f"🚫 BLOCKED MALICIOUS INJECTION: {text}")
            return False
    except Exception:
        return False

    # 2. Check Domain if it acts as a URL
    ...
    try:
        target_url = text
        if target_url.lower().startswith("www."): target_url = "https://" + target_url
